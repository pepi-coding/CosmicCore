...
describe('local collection and economy',()=>{
  it('migrates v1 mass and statistics without resetting progress',()=>{const p=migrateProfile({schemaVersion:1,maximumMass:321,statistics:{wins:7}});expect(p.schemaVersion).toBe(3);expect(p.maximumMass).toBe(321);expect(p.statistics.wins).toBe(7);expect(p.ownedManifestations).toEqual(['vessel']);});
