...
import { tickStability, timeout } from '../src/game/systems/StabilitySystem';
...
describe('mass accounting', () => {
  it('redistribution obeys speed, bounds and conservation with stored objects', () => {
    ...
    for (let i = 0; i < 1000; i++) { redistribute(m, i % 2 ? 2 : -1, B.step); expect(m.coreMass + m.fieldMass + m.orbitingStoredMass).toBeCloseTo(m.combatMass); expect(m.distribution).toBeGreaterThanOrEqual(0); expect(m.distribution).toBeLessThanOrEqual(1); }
  });
  ...
  it('compression increases near pull, reduces radius and releases excess orbit mass', () => {
    ...
    const ds = sim.debris.slice(0, 5); ds.forEach(d => d.owner = p.id); p.mass.combatMass += 12.5; p.mass.orbitingStoredMass = 12.5;
    const total = p.mass.combatMass; orbit(p, ds, B.step);
    expect(ds.filter(d => d.owner === p.id)).toHaveLength(2); expect(ds.filter(d => d.owner === null).reduce((n, d) => n + d.mass, p.mass.combatMass)).toBeCloseTo(total);
  });
...
describe('survival and progression', () => {
  it('requires a subsequent direct hit to collapse an exposed Core', () => { const a = new Simulation('training', 100).player; damage(a, 80, true); expect(a.stability.phase).toBe('unstable'); expect(a.alive).toBe(true); damage(a, 5, false); expect(a.alive).toBe(true); damage(a, 1, true); expect(a.stability.phase).toBe('collapsed'); });
  it('recovers without creating mass', () => { const a = new Simulation('training', 100).player; damage(a, 60, true); expect(a.stability.phase).toBe('unstable'); const mass = a.mass.combatMass; tickStability(a, 5); expect(a.stability.phase).toBe('stable'); expect(a.mass.combatMass).toBe(mass); expect(a.mass.coreMass + a.mass.fieldMass).toBeCloseTo(mass); });
  ...
  it('timeout uses phase then protected mass then controlled mass then sudden death', () => {
    const [a, b] = new Simulation('pvp', 100).actors; expect(timeout(a, b)).toBeNull();
    a.mass.combatMass += 1; expect(timeout(a, b)).toBe(a.id);
    b.mass.coreMass += 2; expect(timeout(a, b)).toBe(b.id);
    b.stability.phase = 'unstable'; expect(timeout(a, b)).toBe(a.id);
  });
});
describe('integrated vertical slice', () => {
  it('can naturally capture five debris objects while expanded', () => {
    const sim = new Simulation('training', 100); sim.actors[1].alive = false;
    let maximumOwned = 0;
  ...
  it('is reproducible and maintains mass invariants under combat', () => {
    ...
    for (let i = 0; i < 1800; i++) {
      ...
      for (const actor of a.actors) { const m = actor.mass; expect(m.coreMass + m.fieldMass + m.orbitingStoredMass).toBeCloseTo(m.combatMass); expect(m.combatMass).toBeGreaterThanOrEqual(0); expect(m.orbitingStoredMass).toBeCloseTo(a.debris.filter(d => d.owner === actor.id).reduce((sum, d) => sum + d.mass, 0)); }
    }
  ...
  it('resolves bot collapse and match timeout', () => { const s = new Simulation('pvp', 100); s.actors[1].alive = false; s.step(idle); expect(s.result?.won).toBe(true); const t = new Simulation('pvp', 100); t.time = B.pvp.duration; t.player.stability.trauma = 3; t.step(idle); expect(t.result).not.toBeNull(); });
});
