...
export function migrateProfile(value: unknown): PlayerProfile {
  ...
  if (raw.schemaVersion !== 1 && raw.schemaVersion !== 2) return clean;
  ...
  if (Array.isArray(raw.firstClears)) clean.firstClears = [...new Set(raw.firstClears.filter((id): id is string => typeof id === 'string' && id === 'silent-orbit:normal'))];
