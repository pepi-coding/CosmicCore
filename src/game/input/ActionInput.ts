...
export const bindings = { up: 'KeyW', down: 'KeyS', left: 'KeyA', right: 'KeyD', impulse: 'Space', pulse: 'KeyQ', pull: 'KeyE', orbit: 'KeyR' };
export class ActionInput {
  keys = new Set<string>(); pointer: Vec = { x: innerWidth / 2 + 100, y: innerHeight / 2 }; firing = false;
  touchMove: Vec = { x: 0, y: 0 }; touchAim: Vec | null = null; touchCast = false;
  ...
  attachTouch(root: HTMLElement): void {
    ...
    root.querySelectorAll<HTMLElement>('[data-action]').forEach(button => {
      ...
      this.on(button, 'pointerdown', (e: PointerEvent) => { e.preventDefault(); button.setPointerCapture(e.pointerId); this.pressed.add(action); });
      const end = () => { this.pressed.delete(action); };
  ...
  sample(center: Vec, dt: number): Actions {
    ...
    return { move, aim: this.touchAim && Math.hypot(this.touchAim.x, this.touchAim.y) > .1 ? this.touchAim : { x: this.pointer.x - center.x, y: this.pointer.y - center.y }, aimAssist: this.touchCast, cast: buttons.primary.held, impulse: buttons.impulse.held, pulse: buttons.pulse.held, surge: buttons.pull.held, orbit: buttons.orbit.held, buttons };
  }
  clear(): void { this.keys.clear(); this.pressed.clear(); this.firing = false; this.touchCast = false; this.touchMove = { x: 0, y: 0 }; }
