...
export function vesselState(a: Actor, victory = false): VesselState {
  ...
  if (a.stability.phase === 'unstable') return 'unstable';
...
export class VesselRig {
  ...
  render(a: Actor, time: number, victory: boolean): void {
    ...
    if (!sprite) { sprite = this.scene.add.sprite(a.x, a.y, 'vessel-down'); sprite.setOrigin(metadata.anchor.x, metadata.anchor.y).setScale(.8); this.sprites.set(a.id, sprite); }
    ...
    sprite.setAlpha(!a.alive && time - a.deathTime > C.feedback.collapseDuration ? .2 : 1);
  }
