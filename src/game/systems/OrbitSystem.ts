...
export function release(a: Actor, d: Debris): void {
  d.owner = null; d.previousOwner = a.id; d.fresh = false; d.capture = {}; d.life = B.mass.fragmentLife; d.vx = a.vx - Math.sin(d.angle) * a.radius * B.orbit.angularSpeed; d.vy = a.vy + Math.cos(d.angle) * a.radius * B.orbit.angularSpeed;
  a.mass.orbitingStoredMass -= d.mass; a.mass.combatMass -= d.mass; distribute(a.mass);
}
...
export function tryCapture(a: Actor, d: Debris, count: number, dt: number): boolean {
  ...
  if (r < a.radius && r > B.orbit.minDistance && Math.hypot(d.vx - a.vx, d.vy - a.vy) < B.orbit.maxSpeed && tangent >= B.orbit.minTangent && count < slots(a) && a.mass.combatMass + d.mass <= a.cap) {
    d.capture[a.id] = (d.capture[a.id] ?? 0) + dt;
    if (d.capture[a.id] >= B.orbit.captureTime) {
      d.owner = a.id; d.angle = Math.atan2(d.y - a.y, d.x - a.x); a.mass.combatMass += d.mass; a.mass.orbitingStoredMass += d.mass;
      if (d.fresh || (d.previousOwner !== null && d.previousOwner !== a.id)) a.mass.unbankedMass += d.mass * (d.fresh ? 1 : B.mass.stolenProgress);
      d.fresh = false; d.capture = {}; distribute(a.mass); return true;
    }
