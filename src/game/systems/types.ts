...
export type Actions = { move: Vec; aim: Vec; distribution: number; cast: boolean; impulse: boolean; pulse: boolean; surge: boolean; orbit?: boolean; buttons?: Buttons };
export type Kind = 'player' | 'bot' | 'drifter' | 'orbiter' | 'leech' | 'wisp' | 'guardian';
export type Actor = Vec & { id: number; kind: Kind; vx: number; vy: number; mass: MassState; stability: StabilityState; targetDistribution: number; radius: number; cooldown: Record<'cast' | 'impulse' | 'pulse' | 'surge' | 'contact' | 'orbit', number>; stagger: number; cap: number; angle: number; alive: boolean; channel: ChannelState; melee: MeleeState; modifiers: Modifiers; manifestationId: string; hitPause: number; hitTime: number; deathTime: number; guardBroken: number; guardCharge: number; channelUsed: boolean; meleeHits: number };
...
export type Result = { won: boolean; reason: string; gained: number; banked: number; lost: number; duration: number; grade?: string; materials?: number; currency?: number; firstClear?: boolean; firstClearMass?: number; dungeonId?: string };
